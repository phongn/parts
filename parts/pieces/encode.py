...
# this is useful for certain i18n cases when you might have a file in a ceratin
...
def _encode_bd(target, source, env):
    ...
    for t,s in zip(target,source):
        #open source
        sf=codecs.open(str(s), "r", src_encode)
        #open target
        tf=codecs.open(str(t), "w", target_encode)
        tmp=sf.read()
        tf.write(tmp)
        sf.close()
        tf.close()
...
def _encode_sf(target, source, env):
    return "Encoding %s from %s to %s in file %s"%(source,env['ENCODE_SOURCE'][0],env['ENCODE_TARGET'][0],target)
