...
class TestWindowsSearchPathSafety:
    def test_win32_without_allusersprofile_does_not_raise(self, resolver, monkeypatch):
        ...
        # localpath + syspath concatenation raises UnboundLocalError.
    ...
    def test_win32_uses_appdata_and_allusersprofile(self, resolver, monkeypatch):
        monkeypatch.setenv('APPDATA', _p(os.sep, 'appdata'))
        monkeypatch.setenv('ALLUSERSPROFILE', _p(os.sep, 'allusers'))
        dirs = _dirs(monkeypatch, 'win32', 'winsys')
        i_user = dirs.index(_p(os.sep, 'appdata', 'parts-site', 'winsys'))
        i_sys = dirs.index(_p(os.sep, 'allusers', 'parts-site', 'winsys'))
        # user locations are searched before the system-wide one
        assert i_user < i_sys
