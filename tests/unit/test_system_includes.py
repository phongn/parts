...
class TestSystemRequirementSets:
    ...
    def test_system_header_default_mirrors_default(self):
        # everything DEFAULT pulls, with the headers swapped. A requirement
        # added to DEFAULT but not here would silently stop reaching the parts
        # that depend with system=True.
        default = set(_set_keys('DEFAULT'))
        system = set(_set_keys('SYSTEM_HEADER_DEFAULT'))
        assert system == (default - {'CPPPATH'}) | {'SYSCPPPATH'}
