...
class TestRedHatToolsetPaths:
    ...
    def test_matches_devtoolset_and_gcc_toolset(self, tmp_path, monkeypatch):
        rh = tmp_path / 'rh'
        for name in ('devtoolset-7', 'gcc-toolset-12', 'gcc-toolset-13', 'gcc-toolset-12-extra',
                     'not-a-toolset', 'llvm-toolset-7'):
        ...
        assert got == expected
        # the default regex must not match llvm-toolset, arbitrary names, or a
        # name that only starts like a toolset (the match is a full match)
        assert not any('llvm-toolset' in p or 'not-a-toolset' in p or '-extra' in p for p in got)
...
class TestScanQuery:
    ...
    def test_plain_tool_in_install_root_wins_its_version(self, tmp_path, monkeypatch):
        # gcc and gcc-13 both report 13.0.0; the unversioned name in the
        # install root is the one kept for that version, whatever order the
        # directory listing comes back in
        binroot = tmp_path / 'bin'
        binroot.mkdir()
        for name in ('gcc', 'gcc-13'):
            (binroot / name).write_text('')
        info = _gnuinfo()
        monkeypatch.setattr(info, 'get_default_ver', lambda path: '13.0.0' if os.path.isfile(path) else None)
        real_listdir = os.listdir
        monkeypatch.setattr(common.os, 'listdir',
                            lambda p: ['gcc', 'gcc-13'] if p == str(binroot) else real_listdir(p))
        found = info.scan_query(str(binroot), opt_scan=False)
        assert found == {'13.0.0': (str(binroot), str(binroot / 'gcc'))}

    def test_found_message_reports_cached_result(self, tmp_path, monkeypatch):
        # a second scan_query returns the cached result; the verbose "Found:"
        # message has to report that result, not an empty scan
        binroot = tmp_path / 'bin'
        binroot.mkdir()
        (binroot / 'gcc-13').write_text('')
        info = self._info_with_stubbed_version(monkeypatch)
        messages = []

        def record(category, sfmt, *args, **kw):
            messages.append(sfmt.format(*args) if args else sfmt)

        monkeypatch.setattr(common.api.output, 'verbose_msgf', record)
        info.scan_query(str(binroot), opt_scan=False)
        info.scan_query(str(binroot), opt_scan=False)
        found = [m for m in messages if m.startswith('Found:')]
        assert len(found) == 2
        assert all('gcc-13' in m for m in found)
