...
def __setup(sconsVersion):
    ...
    if not 'PYTHONPATH' in os.environ:
        os.environ['PYTHONPATH'] = sconsPath + os.pathsep + partsLocalPath
    else:
        os.environ['PYTHONPATH'] += os.pathsep + sconsPath + os.pathsep + partsLocalPath
...
def __reprSConsNodeNode(self):
    return "<{0}.{1} object at 0x{2:x} str(self)={3}>".format(self.__module__,
...
def __reprSCons_Node_NodeInfoBase(self):
    ...
    return "<{0}.{1} object at 0x{2:x} field_list={3}>".format(self.__module__,
...
def __reprSConsNodeFSFS(self):
    return "<{0}.{1} object at 0x{2:x} pathTop={3}>".format(self.__module__,
